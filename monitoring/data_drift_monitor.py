...
def monitor_and_generate():
    ...
    print(
        f"Reference window: {ref_start.date()} → {ref_end.date()} ({len(reference_df)} rows)"
    )
    print(
        f"Current window:   {cur_start.date()} → {latest.date()} ({len(current_df)} rows)"
    )
...
def serve_dashboard():
    ...
    if not os.path.exists(abs_report_path):
        return (f"Report not found at {abs_report_path}. "), 404
...
if __name__ == "__main__":
    ...
    if not ok:
        print(
            "Report generation failed or skipped; server will still serve old report if available."
        )
