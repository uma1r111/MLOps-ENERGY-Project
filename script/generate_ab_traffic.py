...
def simulate_user_satisfaction(
    variant_id: str, latency: float, answer_length: int
) -> float:
    """
...
def main():
    ...
    results_summary = {"total_queries": 0, "by_variant": {}, "avg_satisfaction": {}}
    ...
    for variant_id, count in results_summary["by_variant"].items():
        ...
        avg_sat = sum(results_summary["avg_satisfaction"][variant_id]) / len(
            results_summary["avg_satisfaction"][variant_id]
        )
        print(
            f"  {variant_id:15} {count:3} queries ({percentage:5.1f}%) - Avg Satisfaction: {avg_sat:.2f}/5.0"
        )
