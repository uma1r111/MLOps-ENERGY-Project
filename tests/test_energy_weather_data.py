...
def test_fetch_carbon_intensity_missing_intensity(yesterday_iso):
    ...
    mock_response.json.return_value = {
        "data": [
            ...
            {"from": yesterday_iso},
            {"from": yesterday_iso, "intensity": {"actual": 120, "forecast": 130, "index": "moderate"}}
        ]
    }
...
def test_fetch_carbon_generation_mix():
    ...
    with patch("requests.get", return_value=mock_response):
        ...
        for col in ["uk_gen_biomass_%", "uk_gen_gas_%", "uk_gen_wind_%", "uk_gen_solar_%"]:
...
def test_fetch_carbon_generation_mix_all_fuels():
    ...
    with patch("requests.get", return_value=mock_response):
        ...
        expected_cols = ["datetime", "uk_gen_biomass_%", "uk_gen_imports_%", 
                        "uk_gen_gas_%", "uk_gen_nuclear_%", "uk_gen_solar_%", "uk_gen_wind_%"]
...
def test_fetch_octopus_prices():
    ...
    product_data = {
        "results": [
            {
                "code": "AGILE-18-02-21",
                "links": [{"href": "https://api.octopus.energy/v1/electricity-tariffs/E-1R-AGILE-18-02-21-A/"}]
            }
        ]
    }
...
def test_merge_all_sources_complete(mock_weather_data, mock_aqi_data, mock_carbon_data, mock_genmix_data, mock_price_data):
    merged = merge_all_sources(mock_weather_data, mock_aqi_data, mock_carbon_data, mock_genmix_data, mock_price_data)
    assert not merged.empty
    for col in ["temperature_C", "pm10", "carbon_intensity_actual", "retail_price_£_per_kWh"]:
        assert col in merged.columns


def test_merge_all_sources_misalignment(mock_weather_data, mock_aqi_data, mock_carbon_data, mock_genmix_data, mock_price_data):
    shifted_aqi = mock_aqi_data.copy()
    shifted_aqi["datetime"] += pd.Timedelta(hours=1)
    merged = merge_all_sources(mock_weather_data, shifted_aqi, mock_carbon_data, mock_genmix_data, mock_price_data)
...
def test_append_to_historical_new_file(tmp_path, mock_weather_data):
    ...
    result = append_to_historical(mock_weather_data, save_dir=str(tmp_path), file_name="test.csv")
...
def test_append_to_historical_existing_file(tmp_path, mock_weather_data):
    ...
    result = append_to_historical(new_data, save_dir=str(tmp_path), file_name="test.csv")
...
def test_append_to_historical_duplicate_removal(tmp_path):
    ...
    result = append_to_historical(duplicate, save_dir=str(tmp_path), file_name="test.csv")
