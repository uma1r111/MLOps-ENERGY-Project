...
class ABTestAnalyzer:
    ...
    def t_test_satisfaction(self, variant_a: str, variant_b: str) -> Dict:
        ...
        a_data = self.df[self.df["variant_id"] == variant_a][
            "satisfaction_score"
        ].dropna()
        b_data = self.df[self.df["variant_id"] == variant_b][
            "satisfaction_score"
        ].dropna()
        ...
        for i, var_a in enumerate(variants):
            for var_b in variants[i + 1 :]:
                ...
                if "error" not in result:
                    ...
                    print(
                        f"{var_a:15} vs {var_b:15} | Diff: {result['diff']:+.4f} | p={result['p_value']:.4f} | {sig}"
                    )
    ...
    def recommend_winner(self) -> Dict:
        ...
        for variant in variants:
            ...
            satisfaction_score = (
                ...
                if data["satisfaction_score"].notna().any()
                else 0
            )
        ...
        for variant, score_data in sorted(
            scores.items(), key=lambda x: x[1]["total_score"], reverse=True
        ):
            print(
                f"  {variant:15} Score: {score_data['total_score']:.4f} | "
    ...
    def plot_distributions(self, save_path: str = "monitoring/ab_test_plots.png"):
        ...
        satisfaction_data.boxplot(
            column="satisfaction_score", by="variant_id", ax=axes[0, 1]
        )
