...
def load_rag_system():
    ...
    if not os.path.exists(INDEX_PATH):
        raise FileNotFoundError(
            f"FAISS index not found. Please run 'python src/rag/ingest.py' first."
        )
    ...
    try:
        ...
        if enable_guardrails and guardrail_engine:
            ...
            if not input_validation['passed']:
                ...
                for v in violations:
                    error_msg += f"- {v.get('type', 'Unknown')}: {v.get('message', '')}\n"
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                stats_display = gr.Markdown(
                    label="📊 Statistics",
                    value="Submit a query to see statistics."
                )
