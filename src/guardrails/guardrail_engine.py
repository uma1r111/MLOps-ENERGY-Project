...
class GuardrailEngine:
    ...
    def __init__(
        self, config_path: Optional[str] = None, log_dir: str = "logs/guardrails"
    ):
        """
    ...
    def _load_config(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "policies": {"block_on_violation": True, "log_all_events": True},
        }
    ...
    def validate_input(
        ...
        try:
            ...
            validation_result["injection_detected"] = input_result.get(
                "injection_detected", False
            )
            validation_result["sanitized_input"] = input_result.get(
                "sanitized_input", user_input
            )
            ...
            validation_result["violations"].append(
                {"type": "SYSTEM_ERROR", "message": str(e)}
            )
    ...
    def moderate_output(
        self, model_output: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            moderation_result["toxicity_scores"] = output_result.get(
                "toxicity_scores", {}
            )
            moderation_result["hallucination_detected"] = output_result.get(
                "hallucination_detected", False
            )
            moderation_result["sanitized_output"] = output_result.get(
                "sanitized_output", model_output
            )
    ...
    def _log_event(self, event_type: str, result: Dict[str, Any]) -> None:
        ...
        try:
            ...
            log_file = (
                self.log_dir
                / f"guardrails_{datetime.utcnow().strftime('%Y%m%d')}.jsonl"
            )
            ...
            GuardrailMetrics.record_validation(
                validation_type=event_type, passed=result["passed"]
            )

            # Latency
            GuardrailMetrics.record_latency(
                operation=event_type, latency_ms=result["latency_ms"]
            )

            # Violations
            for violation in result["violations"]:
                GuardrailMetrics.record_violation(violation_type=violation["type"])

            # PII detections (input only)
            if event_type == "input_validation" and result.get("pii_detected"):
                for entity in result["pii_detected"]:
                    GuardrailMetrics.record_pii_detection(entity_type=entity["type"])
    ...
    def get_metrics(self) -> Dict[str, int]:
        ...
        return {
            ...
            "input_violations": sum(
                1
                for k in stats["violation_types"]
                if "PROMPT_INJECTION" in k or "INPUT" in k
            ),
            "output_violations": sum(
                1 for k in stats["violation_types"] if "TOXICITY" in k or "OUTPUT" in k
            ),
