...
class InputValidator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Compiled {len(self.compiled_patterns)} injection detection patterns"
        )
        ...
        logger.info(
            f"Compiled PII patterns for: {list(self.compiled_pii_patterns.keys())}"
        )
    ...
    def _add_custom_recognizers(self):
        ...
        try:
            ...
            phone_patterns = [
                Pattern(
                    name="phone_with_dashes", regex=r"\b\d{3}-\d{3}-\d{4}\b", score=0.85
                ),
                Pattern(
            ...
            ssn_patterns = [
                ...
                Pattern(
                    name="ssn_spaces", regex=r"\b\d{3}\s\d{2}\s\d{4}\b", score=0.85
                ),
            ]
    ...
    def validate(self, user_input: str) -> Dict[str, Any]:
        ...
        if len(user_input) > self.max_length:
            ...
            logger.warning("❌ BLOCKED: Input too long")
            return result
        ...
        if self.enable_pii:
            ...
            logger.info(
                f"  → PII check result: detected={pii_result['detected']}, entities={len(pii_result['entities'])}"
            )
    ...
    def _detect_pii(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"detected": False, "entities": [], "anonymized_text": text}
        ...
        for pattern in self.compiled_patterns:
            try:
                ...
                if match:
                    ...
                    logger.debug(
                        f"✓ Pattern '{pattern.pattern}' matched: '{match.group(0)}'"
                    )
            except Exception as e:
