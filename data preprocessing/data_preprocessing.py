...
new_df["is_night"] = (
    new_df["hour"].between(23, 23) | new_df["hour"].between(0, 6)
).astype(int)
...
new_df["carbon_per_price"] = new_df["carbon_intensity_actual"] / (
    new_df["retail_price_£_per_kWh"] + 1e-6
)
...
# Rolling features
combined_df["price_rolling_24h_mean"] = (
    combined_df["retail_price_£_per_kWh"].rolling(24, min_periods=1).mean()
)
combined_df["price_rolling_24h_std"] = (
    combined_df["retail_price_£_per_kWh"].rolling(24, min_periods=1).std()
)
combined_df["temp_rolling_24h_mean"] = (
    combined_df["temperature_C"].rolling(24, min_periods=1).mean()
)
combined_df["temp_rolling_24h_std"] = (
    combined_df["temperature_C"].rolling(24, min_periods=1).std()
)
combined_df["carbon_rolling_24h_mean"] = (
    combined_df["carbon_intensity_actual"].rolling(24, min_periods=1).mean()
)
combined_df["carbon_rolling_24h_std"] = (
    combined_df["carbon_intensity_actual"].rolling(24, min_periods=1).std()
)
combined_df["wind_rolling_24h_std"] = (
    combined_df["uk_gen_wind_%"].rolling(24, min_periods=1).std()
)
...
lag_rolling_cols = [
    ...
    "temp_rolling_24h_std",
    "carbon_rolling_24h_mean",
    "carbon_rolling_24h_std",
...
print("   - Original: 23")
print("   - Temporal: 13")
print("   - Interactions: 6")
print("   - Log transforms: 8")
print("   - Lag features: 9")
print("   - Rolling features: 8")
print(
    f"   - Scaled features: {len([col for col in final_df.columns if col.startswith('scaled_')])}"
)
